...
void setup() {
    ...
    Particle.function("turnAlarm", turnAlarm);
    Particle.function("ringASecond", ringAFewSecond);
     Particle.function("dismiss", dismiss);
    Particle.function("dismissNOff", dismissNOff);
...
int setTimezone(String timezoneArg){
    ...
    int commaIndex2 = timezoneArg.indexOf(',',commaIndex+1);
    
    int gmt = timezoneArg.substring(0,commaIndex).toInt();
    String dst = timezoneArg.substring(commaIndex+1,commaIndex2);
    String dstEuro = timezoneArg.substring(commaIndex2+1);
    ...
    if(dst == "true"){
        ...
        rtc.setUseEuroDSTRule(false);
    }
    
    rtc.setTimeZone(gmt);
    setTimeOnFPGA("");
    Particle.publish("debug", "run setTimezone, tz="+String(gmt)+" dst="+dst+" dstEuro="+dstEuro);
    return 1;
}
...
int setTestTimeZone(String place){
    ...
    //     default: setTimezone("+7,false,false"); break;
    // }
}
